...
/** ddd:service — Create a new Domain Service in the Domain/Services layer. */
final class DddServiceCommand extends Command
{
    protected $signature = 'ddd:service
                            {domain : Domain name (PascalCase)}
                            {name : Service class name (PascalCase)}
                            {--force} {--dry-run}';
    ...
    public function handle(ClassGenerator $generator): int
    {
        ...
        try {
            $path = $generator->generate('service', $domain, 'Domain', 'Services', $name, [], (bool) $this->option('force'), (bool) $this->option('dry-run'));
            $relative = str_replace(base_path() . '/', '', $path);
            $this->line((bool) $this->option('dry-run') ? "<fg=cyan>[dry-run]</> Would create: {$relative}" : "<fg=green>✓</> Created: {$relative}");
            if (! (bool) $this->option('dry-run')) {
                $this->info("✅ Service '{$name}' created in {$domain}/Domain/Services/");
            }
        } catch (\Throwable $e) {
